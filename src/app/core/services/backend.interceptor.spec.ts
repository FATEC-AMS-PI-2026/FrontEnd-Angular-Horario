...
describe('Conexão futura com o backend', () => {
  ...
  // TEMPORÁRIO: excluir este teste de tokens locais e demo após integrar o backend Java.
  it('não envia identificadores locais ou demonstrativos ao servidor mesmo se habilitado', () => {
    config.habilitado = true;
    for (const token of ['gini-local:perfil', 'gini-demo-local-sem-validade-no-backend']) {
      let erro: unknown;
      client.get('https://backend.test/api/cursos', {
